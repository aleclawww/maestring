...
export default function SignupForm({ referralCode }: SignupFormProps) {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    // Wrapped in try/catch — without it a network error leaves loading=true
    // forever and the user has to hard-refresh. On signup that's worse than
    // login because we'd also miss the `signup_completed` analytics event.
    try {
      ...
      setError(
        'Network error while creating the account. Check your connection and try again.',
      )
    } finally {
      setLoading(false)
    }
  ...
  async function handleGoogleOAuth() {
    ...
    try {
      ...
      setError(
        'Network error while starting Google sign-up. Check your connection.',
      )
      setGoogleLoading(false)
    }
  }
