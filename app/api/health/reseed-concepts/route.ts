...
export async function POST(_req: NextRequest) {
  ...
  const domainRows = DOMAINS.map((d, i) => ({
    ...
    exam_weight_percent: d.examWeightPercent, color: d.color, sort_order: i,
  }));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: domains, error: domErr } = await (supabase.from('knowledge_domains') as any)
    .upsert(domainRows, { onConflict: 'certification_id,slug' }).select('id, slug');
  if (domErr) return NextResponse.json({ stage: 'domains', error: domErr.message }, { status: 500 });
  ...
  const topicRows = TOPICS.map((t, i) => {
    ...
    if (!domainId) throw new Error(`Topic ${t.slug} → unknown domain ${t.domainSlug}`);
  ...
  const { data: topics, error: topErr } = await (supabase.from('domain_topics') as any)
    .upsert(topicRows, { onConflict: 'domain_id,slug' }).select('id, slug');
  if (topErr) return NextResponse.json({ stage: 'topics', error: topErr.message }, { status: 500 });
  ...
  const conceptRows = CONCEPTS.map(c => {
    ...
    if (!domainId) throw new Error(`Concept ${c.slug} → unknown domain ${c.domainSlug}`);
    return {
      certification_id: CERTIFICATION_ID, domain_id: domainId,
      topic_id: topicIdBySlug.get(c.topicSlug) ?? null,
      slug: c.slug, name: c.name, description: c.description, difficulty: c.difficulty,
      key_facts: c.keyFacts, exam_tips: c.examTips,
      aws_services: c.awsServices, confused_with: c.confusedWith, is_active: true,
    };
  });
  ...
  for (let i = 0; i < conceptRows.length; i += BATCH) {
    ...
    if (error) {
      ...
      return NextResponse.json({ stage: 'concepts', error: error.message, totalSoFar: total }, { status: 500 });
    }
    total += batch.length;
  }

  logger.info({ domains: DOMAINS.length, topics: TOPICS.length, concepts: total }, 'reseed complete');
  return NextResponse.json({ ok: true, domains: DOMAINS.length, topics: TOPICS.length, concepts: total });
}
